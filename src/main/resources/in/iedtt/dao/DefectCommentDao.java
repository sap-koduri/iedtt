...
public class DefectCommentDao {
	...
	private Response logComment(DefectComment comment) {
		...
		try {
			String query = "insert into defects (defect_id,commentor,comment,date_time) values (?,?,?,?)";
	...
	public List<DefectComment> getCommentsByDefectId(int defectId){
		...
		System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% log Comment Method called %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
		try {
			String query = "select * from  defects where defect_id = ?";
			connection = DBUtil.getconnection();
			pstmt = connection.prepareStatement(query);
			...
			if(rs != null){
				while(rs.next()) {
					...
					comment.setDateTime(rs.getString(4));
